...
if [ "$totalAlerts" -gt "0" ]
    then
		...
		echo "$(tput -T linux setaf 3)Your project contains at least one high-priority JUnit-related PMD alert (this means that your tests are missing things like the @Test annotation, or assertions).  Check the \"High-Priority JUnit PMD Warnings\" link on the left side for details.  You need to fix these warnings before any of the teaching staff test cases will be run$(tput -T linux sgr 0)".
        exit 1
fi
...
do
        ...
        if [[ "$class" != "$HEADER" && "$package" != *"ui" && "$class" != *"Test" && "$class" != *".new Observer() {...}" ]]
            then
				...
				echo "$class $linePercentClean" >> coverage_report.txt
        fi
done < $INPUT
IFS=$OLDIFS
...
if [ "$CLASSCOUNTER" -gt "0" ]
    then
        echo "$(tput -T linux setaf 3)TOTAL of $CLASSCOUNTER classes not meeting the $ARG% line coverage threshold.  Not all teaching staff testcases will be run $(tput -T linux sgr 0)"
fi
